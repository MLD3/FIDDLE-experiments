...
def resolve_duplicates_discrete(data):
    ...
    if parallel:
        df_dedup = Parallel(n_jobs=64, verbose=10)(delayed(_resolve_duplicates_impl)(v, df_v) for v, df_v in all_dups.groupby('variable_name'))
    else:
    ...
    for table_name, df_table in data.items():
        if 't' in df_table.columns:
            try:
                ...
                print('{} contains duplicate records'.format(table_name))
                data[table_name] = df_table.drop_duplicates(subset=['ID', 't', 'variable_name'])
...
def resolve_duplicates_continuous(data):
    ...
    if parallel:
        df_dedup = Parallel(n_jobs=64, verbose=10)(delayed(resolve_duplicates_inputevents)(v, df_v) for v, df_v in all_dups.groupby('variable_name'))
    else:
