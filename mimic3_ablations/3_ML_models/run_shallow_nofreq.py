...
print('EXPERIMENT:', 'nofreq,model={},outcome={},T={},dt={}'.format(model_name, task, T, dt))

# Create checkpoint directories
import pathlib
pathlib.Path("./checkpoint/nofreq,model={},outcome={},T={},dt={}/".format(model_name, task, T, dt)).mkdir(parents=True, exist_ok=True)

######
# Data
import lib.data as data
if task == 'mortality':
    tr_loader, va_loader, te_loader = data.get_benchmark_splits_nofreq(dt, fuse=True)
else:
    assert False
...
try:
    np.savetxt(
        'output/nofreq,outcome={},T={},dt={}/{},coef.txt'.format(task, T, dt, model_name), 
...
evaluate.save_test_predictions(y_te, yte_pred, model_type, './output/nofreq,outcome={},T={},dt={}/'.format(task, T, dt))
