...
def transform_time_series_table(df_in, args):
    ...
    if parallel:
        out = dict(Parallel(n_jobs=n_jobs, verbose=10)(
            delayed(func_encode_single_time_series)(i, g, variables, variables_num_freq, args.T, args.dt, args.stats_functions, impute=False)
            for i, g in grouped[:N]
        ))
        
    else:
        out = dict(
            func_encode_single_time_series(i, g, variables, variables_num_freq, args.T, args.dt, args.stats_functions, impute=False) 
